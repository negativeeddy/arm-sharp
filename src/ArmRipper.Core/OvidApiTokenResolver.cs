...
/// <summary>
/// Resolves the OVID API token from the effective (DB-first) settings via
...
public sealed class OvidApiTokenResolver : IOvidApiTokenSource
{
    ...
    public OvidApiTokenResolver(IServiceScopeFactory scopeFactory)
    {
    ...
    public string? GetToken()
    {
        try
        {
            ...
            return settings.GetEffectiveAsync().GetAwaiter().GetResult().OvidApiToken;
        }
        catch
        {
            return null;
